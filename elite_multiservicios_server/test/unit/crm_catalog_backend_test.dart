...
void main() {
  group('CRM Dynamic Catalog & Quote Snapshot Integrity Tests', () {
    ...
    test('CrmServiceLine instantiates with proper attributes and serialization', () {
      final now = DateTime.now().toUtc();
    ...
    test('CrmCatalogItem models calculation rules and pricing correctly', () {
      final now = DateTime.now().toUtc();
      final metadataJson = jsonEncode({'hoursPerShift': 12, 'daysPerMonth': 26});
    ...
    test('CrmCatalogItemScope models sector-differentiated pricing overrides', () {
      final now = DateTime.now().toUtc();
    ...
    test('CrmQuoteItem stores inmutable snapshot of catalog pricing and formula', () {
      final now = DateTime.now().toUtc();
    ...
    test('AppPermissions catalog contains canonical catalog RBAC permissions', () {
